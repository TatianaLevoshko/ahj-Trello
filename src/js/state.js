export default class TrelloState {
  ...
  moveDataItem(draggableCard, siblingData) {
    if (siblingData) {
      [this.data['todo'], this.data['in-progress'], this.data['done']].forEach(
        (items) => {
          const draggableIndex = items.findIndex(
            item => item.id === draggableCard.dataset.id,
          );
          ...
          if (draggableIndex !== -1) {
            ...
            if (siblingData.cardId) {
              const siblingIndex = this.data[siblingData.columnName].findIndex(
                (item) => {
                  return item.id === siblingData.cardId;
                },
              );

              this.data[siblingData.columnName].splice(
                siblingIndex,
                0,
                draggableItem,
              );
