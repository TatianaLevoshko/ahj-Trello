...
export default class Trello {
  ...
  addGhostCard(event) {
    ...
    const elementCurrent = document.elementFromPoint(
      event.clientX,
      event.clientY,
    );
    ...
    if (elementCurrent.closest(Trello.columnBodySelector)) {
      if (!elementCurrent.closest(Trello.columnBodySelector).hasChildNodes()) {
        elementCurrent
          .closest(Trello.columnBodySelector)
          .appendChild(this._cloneCard);
      }
      else if (
        targetCard
        && targetCard.dataset.id !== this._draggableCardId
      ) {
        ...
        if (
          ...
          > window.scrollY
          + top
          + elementCurrent.closest(Trello.columnCardSelector).offsetHeight / 2
        ) {
          elementCurrent
            .closest(Trello.columnBodySelector)
            .insertBefore(
        ...
        const elementAbove = document.elementFromPoint(
          event.clientX,
          event.clientY - 30,
        );
  ...
  onClickColumnBtnAdd(event) {
    [...this._container.querySelectorAll(Trello.columnFooterSelector)].forEach(
      (columnFooter) => {
        columnFooter
          .querySelector(Trello.columnBtnAddSelector)
          .classList.remove('hidden');
        columnFooter
          .querySelector(Trello.columnFormSelector)
  ...
  onMouseDown(event) {
    ...
    const draggableCardCoordinates
      = this._draggableCard.getBoundingClientRect();
  ...
  onMouseUp(event) {
    ...
    if (
      !this._draggableCard
      || elementCurrent.closest(Trello.cardBtnDeleteSelector)
    ) {
      return;
    }
    ...
    if (
      ...
      if (this._cloneCard.nextElementSibling) {
        siblingData = {
          ...
          columnName: this._cloneCard.closest(Trello.columnBodySelector).dataset
            .name,
        };
      }
        ...
        if (
          ...
          siblingData = {
            cardId: elementCurrent.closest(Trello.columnCardSelector)
              .nextElementSibling.dataset.id,
            columnName: elementCurrent.closest(Trello.columnBodySelector)
              .dataset.name,
          };
        }
        else {
  ...
  onSubmitForm(event) {
    ...
    const textarea = this._activeForm.querySelector(
      Trello.formTextareaSelector,
    );
  ...
  redrawColumnBody(columnBody) {
    ...
    this._state.data[columnBody.dataset.name].forEach((element) => {
      columnBody.insertAdjacentHTML(
        'beforeend',
        Trello.markupCard(element.id, element.title),
      );
    });
  }
  ...
  redrawDOM() {
    ...
    this.redrawColumnBody(
      this._container.querySelector('[data-name="in-progress"]'),
    );
  ...
  static get boardColumnSelector() {
    return '.trello-board__column';
  }

  static get boardSelector() {
    return '.trello__board';
  }

  static get cardBtnDeleteSelector() {
    return '.trello-card__btn-delete';
  }

  static get columnBtnAddSelector() {
    return '.trello-column__btn-add';
  }

  static get columnBodySelector() {
    return '.trello-column__body';
  }

  static get columnCardSelector() {
    return '.trello-column__card';
  }

  static get columnFooterSelector() {
    return '.trello-column__footer';
  }

  static get columnFormSelector() {
    return '.trello-column__form';
  }

  static get columnHeaderSelector() {
    return '.trello-column__header';
  }

  static get formBtnResetSelector() {
    return '.trello-form__btn-reset';
  }

  static get formBtnSubmitSelector() {
    return '.trello-form__btn-submit';
  }

  static get formTextareaSelector() {
    return '.trello-form__textarea';
  }
